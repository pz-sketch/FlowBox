...
extension SettingsWindowController {
    func buildPresenceTab(tabView: NSTabView) {
        ...
        presenceCheck = UIStyle.switchRow(L10n.tr("离开自动锁屏(无操作超时后确认,无人则锁)", "Auto-lock when away (checks after idle, locks if empty)"), target: self, action: #selector(togglePresence))
        ...
        presenceStatusHint = UIStyle.hint("")
        ...
        lockRow.addArrangedSubview(UIStyle.controlLabel(
            L10n.tr("无操作多久后检测(秒)", "Idle before checking (s)"), width: UIStyle.Metrics.labelColumnWidth
        ))
        ...
        confirmRow.addArrangedSubview(UIStyle.controlLabel(
            L10n.tr("检测间隔(每多久复查,秒)", "Recheck every (s)"), width: UIStyle.Metrics.labelColumnWidth
        ))
        ...
        graceRow.addArrangedSubview(UIStyle.controlLabel(
            L10n.tr("开启/解锁后宽限(秒)", "Grace after unlock (s)"), width: UIStyle.Metrics.labelColumnWidth
        ))
        ...
        presenceStrangerCheck = UIStyle.switchRow(L10n.tr("陌生人也锁屏(需先注册主人脸)", "Lock on unfamiliar faces (enroll owner first)"), target: self, action: #selector(togglePresenceStranger))
        ...
        presenceOwnerStatusLabel = UIStyle.hint("")
        strangerRow.addArrangedSubview(presenceOwnerStatusLabel)
        inner.addArrangedSubview(strangerRow)

        presenceSaveCheck = UIStyle.switchRow(L10n.tr("锁屏前保存摄像头快照(排查误锁用)", "Save camera snapshot before locking (debug)"), target: self, action: #selector(togglePresenceSave))
        ...
        inner.addArrangedSubview(UIStyle.hint(
            L10n.tr(
            "说明:无操作满第 1 个秒数后开摄像头检测人脸;确认有人且仍无操作,每隔第 2 个秒数复查一次;一旦有操作重新累计第 1 个秒数。锁屏后看守休眠,必须输密码或 Touch ID 解锁(macOS 安全限制)。录屏占用摄像头时看守自动让路。",
            "After idle past the 1st value the camera checks your face; if you're there and still idle, it rechecks every (2nd value); any input restarts the 1st countdown. After locking you must enter your password or Touch ID — macOS never lets apps unlock for you. The watch yields while recording uses the camera."
        )))
    ...
    @objc func togglePresence(_ sender: UIStyle.SwitchRow) {
    ...
    @objc func togglePresenceSave(_ sender: UIStyle.SwitchRow) {
        config.presence.saveCaptureOnLock = sender.state == .on
        save()
    }

    @objc func togglePresenceStranger(_ sender: UIStyle.SwitchRow) {
        let on = sender.state == .on
        if on {
    ...
    /// 主人脸注册状态行:未注册/已注册/注册中（状态用颜色表达，不往文案里塞 emoji）
    func updateOwnerStatusLabel() {
        ...
        if pc.ownerFaceprint != nil {
            ...
            presenceOwnerStatusLabel.textColor = UIStyle.Palette.success
            ...
            presenceOwnerStatusLabel.textColor = UIStyle.Palette.textSecondary
        }
    }
}
